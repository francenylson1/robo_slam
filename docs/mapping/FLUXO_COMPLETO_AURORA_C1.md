...
## ⚙️ Nova Automação: Aurora Mapping Studio CLI

> Utilize sempre que possível antes de recorrer aos scripts legados abaixo.

### Comando base
```
python3 src/main_mapping.py --pipeline <pipeline> --input <origem> --output <destino> [--steps ...] [--metadata ...]
```

- `--pipeline`:  
  - `aurora_to_c1` → executa a sequência completa (captura → refinamento → mapa 2D → conversão C1 → POIs → export).  
  - `inventory_snapshot` → gera inventário JSON/Markdown do conteúdo em `mapas/`.
- `--steps`: limita quais etapas rodar (ex.: `--steps refinement,map2d`).
- `--metadata`: caminho para JSON com parâmetros extras (ex.: resolução desejada, limites de filtro).

### Exemplos
```
# Inventariar rapidamente mapas existentes
python3 src/main_mapping.py --pipeline inventory_snapshot --input mapas --output docs/mapping

# Rodar pipeline completo do Aurora com saída em data/pipeline_runs/2025-11-24
python3 src/main_mapping.py --pipeline aurora_to_c1 \
    --input mapas/legacy/originais_aurora \
    --output data/pipeline_runs/2025-11-24

# Reexecutar apenas a geração 2D e export
python3 src/main_mapping.py --pipeline aurora_to_c1 \
    --input mapas/legacy/originais_aurora/sala-maker-1.stcm \
    --output data/pipeline_runs/reprocesso \
    --steps map2d,export
```

As etapas individuais atualmente exibem mensagens de `NotImplementedError` enquanto cada módulo é desenvolvido; entretanto, a orquestração já está pronta e a CLI substitui gradualmente os scripts manuais descritos mais abaixo.

- As configurações padrão residem em `config/mapping.json`. Para customizar temporariamente, gere um JSON de overrides e passe via `--metadata caminho/custom.json`.
