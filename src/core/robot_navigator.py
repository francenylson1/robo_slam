...
class RobotNavigator:
    ...
    def update(self):
        ...
        print(f"DEBUG: Pos: ({self.current_position[0]:.2f}, {self.current_position[1]:.2f}), "
              f"Ângulo: {self.current_angle:.2f}°, "
              f"Alvo: ({self.current_target[0]:.2f}, {self.current_target[1]:.2f}), "
        ...
        # Se estiver muito longe do alvo, ajusta o ângulo primeiro
        if abs(angle_diff) > NAVIGATION_ANGLE_TOLERANCE:
            # Ajusta a velocidade de giro baseado na diferença de ângulo
            turn_value = min(1.0, abs(angle_diff) / 30.0)  # Reduzido de 45 para 30 para giro mais rápido
            if angle_diff < 0:
                turn_value = -turn_value
            self.motors.set_speed(-turn_value * 100, turn_value * 100)
            print(f"DEBUG: Ajustando ângulo - Velocidade de giro: {turn_value:.2f}")
        # Se o ângulo estiver correto, move em direção ao alvo
        elif distance > NAVIGATION_GOAL_TOLERANCE:
            # Velocidade baseada na distância, mas limitada
            forward_value = min(1.0, distance / 1.5)  # Reduzido de 2.0 para 1.5 para aceleração mais rápida
            self.motors.set_speed(forward_value * 100, forward_value * 100)
            print(f"DEBUG: Movendo para alvo - Velocidade: {forward_value:.2f}")
        else:
            # Chegou ao alvo atual
            print(f"DEBUG: Chegou ao alvo atual: {self.current_target}")
            self.motors.stop()
