...
class RobotNavigator:
    ...
    def _move_towards_target(self):
        """Move o robô em direção ao alvo atual com sistema de precisão avançado"""
        ...
        # Calcula distâncias separadas para cada eixo
        distance_x = abs(dx)
        distance_y = abs(dy)
        ...
        # 🚶 LOGS DETALHADOS DE MOVIMENTO
        print("🚶 MOVIMENTO DETALHADO:")
        print("=" * 60)
        print(f"🎯 Estado: {self.navigation_state}")
        print(f"🏠 Retornando à base: {self.is_returning_to_base}")
        print(f"📍 De: ({self.current_position[0]:.4f}, {self.current_position[1]:.4f})")
        print(f"🎯 Para: ({self.current_target[0]:.4f}, {self.current_target[1]:.4f})")
        print(f"📏 Distância total: {distance:.4f}m ({distance*100:.1f}cm)")
        print(f"📊 Distância X: {distance_x:.4f}m ({distance_x*100:.1f}cm)")
        print(f"📊 Distância Y: {distance_y:.4f}m ({distance_y*100:.1f}cm)")
        print(f"🧭 Diferença angular: {angle_diff:.2f}°")
        print(f"➡️ Delta X: {dx:+.4f}m ({'direita' if dx > 0 else 'esquerda' if dx < 0 else 'centrado'})")
        print(f"⬆️ Delta Y: {dy:+.4f}m ({'cima' if dy > 0 else 'baixo' if dy < 0 else 'centrado'})")
        print("=" * 60)
        
        # Sistema de navegação com precisão avançada
        if self.navigation_state == "NAVIGATING_TO_DESTINATION":
            # **FASE 1: APROXIMAÇÃO GROSSEIRA** (distância > 0.5m)
            if distance > 0.5:
                print("DEBUG: === FASE 1: APROXIMAÇÃO GROSSEIRA ===")
                angle_tolerance = NAVIGATION_ANGLE_TOLERANCE  # 3 graus
                ...
                if abs(angle_diff) > angle_tolerance:
                    ...
                    print(f"DEBUG: Ajustando ângulo grosseiro: turn_value = {turn_value:.3f}")
                    self.motors.set_speed(-turn_value * 100, turn_value * 100)
                else:
                    # Move com velocidade moderada
                    forward_value = min(0.4, distance / 2.0)
                    print(f"DEBUG: Movimento grosseiro: {forward_value:.3f}")
                    
            # **FASE 2: APROXIMAÇÃO FINA** (0.15m < distância <= 0.5m)
            elif distance > NAVIGATION_FINE_APPROACH_DISTANCE:
                print("DEBUG: === FASE 2: APROXIMAÇÃO FINA ===")
                angle_tolerance = 2.0  # Tolerância mais restrita
                
                if abs(angle_diff) > angle_tolerance:
                    turn_value = min(0.3, abs(angle_diff) / 45.0)
                    if angle_diff < 0:
                        turn_value = -turn_value
                    print(f"DEBUG: Ajustando ângulo fino: turn_value = {turn_value:.3f}")
                    self.motors.set_speed(-turn_value * 100, turn_value * 100)
                else:
                    forward_value = min(0.2, distance / 1.5)
                    print(f"DEBUG: Movimento fino: {forward_value:.3f}")
                    
            # **FASE 3: AJUSTE DE PRECISÃO** (0.08m < distância <= 0.15m)
            elif distance > NAVIGATION_PRECISION_APPROACH_DISTANCE:
                print("DEBUG: === FASE 3: AJUSTE DE PRECISÃO ===")
                angle_tolerance = 1.0  # Muito preciso
                
                if abs(angle_diff) > angle_tolerance:
                    turn_value = min(0.15, abs(angle_diff) / 60.0)
                    if angle_diff < 0:
                        turn_value = -turn_value
                    print(f"DEBUG: Ajustando ângulo precisão: turn_value = {turn_value:.4f}")
                    self.motors.set_speed(-turn_value * 100, turn_value * 100)
                else:
                    # Velocidade muito baixa para precisão
                    forward_value = min(0.1, distance / 1.0)
                    print(f"DEBUG: Movimento precisão: {forward_value:.4f}")
                    
            # **FASE 4: APROXIMAÇÃO FINAL ESTÁVEL** (distância <= 0.08m)
            else:
                print("DEBUG: === FASE 4: APROXIMAÇÃO FINAL ESTÁVEL ===")
                return self._stable_final_approach()
                
        else:
            # **NAVEGAÇÃO DE RETORNO À BASE** - Velocidade padrão otimizada
            print("DEBUG: === NAVEGAÇÃO DE RETORNO ===")
            angle_tolerance = NAVIGATION_ANGLE_TOLERANCE
            
            if abs(angle_diff) > angle_tolerance:
                turn_value = min(0.4, abs(angle_diff) / 25.0)
                if angle_diff < 0:
                    turn_value = -turn_value
                print(f"DEBUG: Ajuste ângulo retorno: turn_value = {turn_value:.3f}")
                self.motors.set_speed(-turn_value * 100, turn_value * 100)
            else:
                forward_value = min(0.5, distance / 1.8)
                print(f"DEBUG: Movimento retorno: {forward_value:.3f}")
                
        # **APLICAÇÃO DOS COMANDOS DE MOVIMENTO**
        if forward_value > 0:
            # Ajuste fino do ângulo durante o movimento (apenas se necessário)
            ...
            print(f"DEBUG: Comandos finais - forward: {forward_value:.4f}, turn: {turn_value:.4f}")
            print(f"DEBUG: Velocidades motores - esquerdo: {left_speed:.1f}%, direito: {right_speed:.1f}%")
    ...
    def _stable_final_approach(self):
        """Sistema de aproximação final estável - evita oscilações"""
